...
- Listen1 导入歌曲播放时会尝试匹配 Mineradio 已支持平台的同名同歌手可播版本，不使用 Listen1 旧直链。
...
- 推荐下载：`Mineradio-1.1.2-Setup.exe`
...
Listen1 兼容功能仅用于读取用户自己导出的 `listen1_backup.json`，把其中的歌单和歌曲元数据导入本地。导入时会丢弃备份中的旧播放直链；播放时只通过 Mineradio 当前已支持的网易云 / QQ 搜索与播放流程尝试匹配可用版本。若目标歌曲受会员、版权、地区或平台登录限制，应用会提示、换源或跳过，不会绕过限制。
