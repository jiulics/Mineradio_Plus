...
## Mineradio Plus 二次开发说明

本仓库是 `jiulics/Mineradio_Plus` 二次开发版本，基于原创项目 [XxHuberrr/Mineradio](https://github.com/XxHuberrr/Mineradio) 继续优化。原始创意、整体产品方向、核心视觉体验与基础工程均来自原作者 XxHuberrr，在此向原创作者和早期共创/测试用户致敬。

Plus 版本目前重点优化：

- 首页从占位施工区升级为本地时间 + 天气主视觉卡。
- 天气定位优先使用 GPS/系统定位，失败时提示手动更新，并以 `东莞市松山湖` 作为默认城市兜底。
- 天气卡复用原有 Open-Meteo 天气电台接口，不额外引入新的天气后端。
- 天气氛围条增加音轨同步效果，会跟随当前播放的 bass、beat pulse 和整体能量呼吸。
- 右侧 3D 歌单详情在播放时会自动同步到当前曲目，打开详情后也会定位到正在播放的歌曲。
- 在歌单详情中点击播放后，不再自动退出详情视图，方便边听边浏览当前歌单。
- 增加轻量回归检查脚本 `scripts/check-home-weather-sync.js`，用于防止首页天气和右侧同步逻辑回退。

> 说明：本仓库为二次开发分支，不代表原作者官方发布版本。正式版权、授权和原创归属请以原项目和本仓库的 LICENSE / NOTICE 为准。
...
Mineradio Plus 二次开发版继续基于原项目探索首页天气、播放同步和桌面端体验优化。再次感谢 XxHuberrr 开源 Mineradio，让后续二次开发和学习实践有了可靠起点。
